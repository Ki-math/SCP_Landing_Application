...
効果の実測（クロス風 地上5→上空10.6 m/s、falcon9、磨きパス・追従強化込みの
現行既定）: 計画残差 **0.1 m**、閉ループ **4.2 m**・接地 −3.4 m/s・傾斜 1.0°、
MCS（風＋3σ分散20ラン）は**成功100%**（水平は18/20ランが10 m以内、超過2件は
windScale抽選で実質13〜15 m/s相当の強風ケース。vTd max 5.0・傾斜 max 1.3°）。なお緩和はいずれも「機体にできることを増やす」方向の変更なので、
