...
`scpMCS` が本チューニングを**自動で適用**します（手動の `scpWindTune(prob)` は
不要。二重適用はフラグで防止）。既定の絞った姿勢制約のままだと風トリムも
divert もできず計画残差が10 m級で残るため（実測: 50 mオフセット時 40.8 m →
適用で 1.9 m）、風を織り込む計画では実質必須です。無効化は
`prob.opt.autoWindTune = false;`。手動適用の場合:
...
効果の実測（クロス風 地上4→上空10.6 m/s、falcon9、磨きパス・追従強化込みの
現行既定）: 単発閉ループは水平 **0.73 m**・接地速度 **5.14 m/s**・傾斜 **2.75°**。
MCS（風＋3σ分散100ラン、seed=1）は**成功98%**で、水平誤差は平均1.98 m / 最大18.9 m、
接地速度は平均4.40 m/s / 最大9.37 m/s、傾斜は平均2.45° / 最大9.32°でした。
旧点火則の成功85%に対し、高所停止後の自由落下を抑えて13ポイント改善しています。
残る2件は3σ級の初期位置・速度偏差を含む再計画ケースです。なお緩和はいずれも「機体にできることを増やす」方向の変更なので、
