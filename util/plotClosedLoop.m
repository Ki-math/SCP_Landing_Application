...
%   PLOTCLOSEDLOOP(SOL, CL, CFG, AXS)   AXS (9要素のaxes配列) に描く (GUI統合)
%
%   内容: 高度 / 水平位置(CR,DR) / 速度(全機速・鉛直) / 傾斜成分(ピッチ・ヨー) /
%         角速度 / 推力(大きさ・点火基数) / TVC角 / 舵角偏差(4枚) / 追従QP計算時間
%   実線=閉ループ, 破線=計画.
%
...
if nargin < 4 || isempty(axs)
    ...
    axs = gobjects(1,9);
    for i = 1:9, axs(i) = nexttile(tl); end
end
for i = 1:9, cla(axs(i),'reset');  hold(axs(i),'on');  grid(axs(i),'on'); end
...
xlabel(axs(4),'t [s]'); ylabel(axs(4),'傾斜角成分 [deg]');
legend(axs(4),{'ピッチ (DR方向)','ヨー (CR方向)'},'Location','best');
title(axs(4),'傾斜成分');

%% 5. 角速度
plot(axs(5), cl.t, rad2deg(cl.x(11,:)), '-','Color',[.3 .3 .3],'LineWidth',1.0);
plot(axs(5), cl.t, rad2deg(cl.x(12,:)), 'b-','LineWidth',1.2);
plot(axs(5), cl.t, rad2deg(cl.x(13,:)), 'r-','LineWidth',1.2);
plot(axs(5), sol.t, rad2deg(sol.w(2,:)), 'b--', sol.t, rad2deg(sol.w(3,:)), 'r--','LineWidth',0.9);
xlabel(axs(5),'t [s]'); ylabel(axs(5),'角速度 [deg/s]');
legend(axs(5),{'ロール','ピッチ','ヨー'},'Location','best');
title(axs(5),'角速度 (機体軸)');

%% 6. 推力 (左:大きさ 右:点火基数)
yyaxis(axs(6),'left');
plot(axs(6), cl.t, vecnorm(cl.u(1:3,:))*cfg.Fs/1e6, 'b-','LineWidth',1.4);
stairs(axs(6), tu, sol.Tmag(1:N)/1e6, 'b--','LineWidth',1.0);
ylabel(axs(6),'推力 [MN]');
yyaxis(axs(6),'right');
stairs(axs(6), tu, sol.engSched(1:N), '-','Color',[.85 .4 .1],'LineWidth',1.2);
ylabel(axs(6),'点火基数 (計画)'); ylim(axs(6),[-0.2, max(sol.engSched)+0.8]);
xlabel(axs(6),'t [s]'); title(axs(6),'推力 (左:大きさ 右:基数)');

%% 7. TVC角 (ピッチ/ヨー成分, 符号付き)
...
plot(axs(7), cl.t, gPc, 'b-', cl.t, gYc, 'r-','LineWidth',1.4);
stairs(axs(7), tu, gPp, 'b--','LineWidth',1.0);
stairs(axs(7), tu, gYp, 'r--','LineWidth',1.0);
xlabel(axs(7),'t [s]'); ylabel(axs(7),'TVC角 [deg]');
legend(axs(7),{'ピッチ面 (T_3/T_1)','ヨー面 (T_2/T_1)'},'Location','best');
title(axs(7),'TVC (符号付き偏向角. 非点火区間=0)');
...
for i = 1:4
    hh(i) = plot(axs(8), cl.t, rad2deg(cl.u(3+i,:)), '-','Color',cols(i,:),'LineWidth',1.2);
    stairs(axs(8), tu, rad2deg(sol.uhat(3+i,1:N)), '--','Color',cols(i,:),'LineWidth',0.8);
end
xlabel(axs(8),'t [s]'); ylabel(axs(8),'舵角偏差 [deg]');
legend(axs(8), hh, names, 'Location','best');
title(axs(8),'舵面 (トリムからの偏差)');

%% 9. 追従QPの計算時間
histogram(axs(9), cl.qpT*1e3, 30);
xlabel(axs(9),'追従QP時間 [ms]'); ylabel(axs(9),'回数');
title(axs(9), sprintf('MPC %d回 平均%.1fms 最大%.1fms 収束%.0f%%', numel(cl.qpT), ...
